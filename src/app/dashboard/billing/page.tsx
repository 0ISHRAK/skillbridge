...
export default function BillingPage() {
  ...
  useEffect(() => {
    const loadWallet = async () => {
      try {
        ...
        // The dashboard remains usable while the wallet request retries on refresh.
      }
    };
    void loadWallet();
  }, []);
  ...
  const handlePaymentSubmit = async (e: React.FormEvent) => {
    ...
    if (step === 1) {
      ...
      // Record the completed sandbox payment server-side before updating the UI.
      if (selectedPack) {
        const res = await fetch("/api/wallet", {
          method: "POST",
          ...
          body: JSON.stringify({
            ...
            gatewayTxnId: `${Date.now()}-${Math.random().toString(36).slice(2, 10)}`,
          }),
        });
        const data = await res.json();
        if (!res.ok) {
          setCheckoutError(data.error || "Unable to complete payment.");
          return;
        }
      ...
      {showCheckout && selectedPack && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="max-w-md w-full bg-card border border-border rounded-2xl shadow-2xl p-6 relative overflow-hidden animate-scale-up">
            <div className="absolute top-0 left-0 right-0 h-1 bg-linear-to-r from-pink-500 via-red-500 to-amber-500" />
