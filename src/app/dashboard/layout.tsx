  ...
  useEffect(() => {
    fetch("/api/auth/me")
      ...
      .then((user) => {
        ...
        if (user.role === "mentor" && !user.isMentorApproved && pathname !== "/dashboard") {
          router.push("/auth/mentor-pending");
          return;
        }
        if (user.role === "learner" && pathname.includes("/dashboard/mentor")) {
          router.push("/dashboard");
          return;
        }
        if (user.role === "mentor" && (pathname === "/dashboard/courses" || pathname === "/dashboard/book")) {
          router.push("/dashboard");
          return;
        }
    ...
    <div className="flex-1 flex flex-col md:flex-row bg-background">
      ...
      <header className="md:hidden flex items-center justify-between px-4 h-16 border-b border-border bg-card">
        <Link href="/" className="flex items-center gap-1.5">
          <div className="w-7 h-7 rounded-lg bg-linear-to-tr from-primary to-secondary flex items-center justify-center text-white text-xs font-bold">
      ...
      <aside className="hidden md:flex md:w-64 bg-card border-r border-border flex-col justify-between p-6 shrink-0 relative z-30">
        <div className="space-y-8">
          ...
          <Link href="/" className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-linear-to-tr from-primary to-secondary flex items-center justify-center text-white font-bold">
