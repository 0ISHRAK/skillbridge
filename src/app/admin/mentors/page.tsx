...
interface Mentor {
  ...
  mentorApplicationStatus: "not_submitted" | "pending" | "approved" | "rejected";
  skills: string | null;
  hourlyRate: number;
...
export default function AdminMentorsPage() {
  ...
  const handleVerify = async (mentorId: string, approve: boolean) => {
    ...
    try {
      ...
      if (res.ok) {
        setMentors((prev) =>
          prev.map((m) => (m.id === mentorId ? {
            ...m,
            isMentorApproved: approve,
            mentorApplicationStatus: approve ? "approved" : "rejected",
          } : m))
        );
      }
    } catch (err) {
  ...
  const filtered = mentors.filter((m) => {
    if (filter === "pending") return m.mentorApplicationStatus === "pending";
    if (filter === "approved") return m.isMentorApproved;
    if (filter === "rejected") return m.mentorApplicationStatus === "rejected";
    return true;
  });

  const pendingCount = mentors.filter((m) => m.mentorApplicationStatus === "pending").length;
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-2">
        {(["all", "pending", "approved", "rejected"] as const).map((f) => (
          ...
          >
            {f === "all" ? "All Mentors" : f === "pending" ? "Pending" : f === "approved" ? "Approved" : "Rejected"}
          </button>
        ))}
      </div>
            ...
            >
              <div className="flex items-start justify-between">
                ...
                <span className={`px-2 py-0.5 rounded-full text-[8px] font-bold uppercase ${
                  ...
                  {mentor.mentorApplicationStatus.replace("_", " ")}
                </span>
              </div>
              ...
              <div className="mt-4 space-y-2 text-[10px] text-muted-foreground">
                ...
                <div>
                  ...
                  <span className="font-semibold text-foreground">
                    {mentor.skills ? JSON.parse(mentor.skills).join(", ") : "Not provided"}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Rate:</span>
                ...
                {mentor.linkedinUrl && (
                  <a href={mentor.linkedinUrl} target="_blank" rel="noreferrer" className="block text-primary hover:underline">
              ...
              <div className="mt-4 flex gap-2">
                {mentor.mentorApplicationStatus === "pending" ? (
                  <>
                    <button
                  ...
                  <button
                    ...
                    className="flex-1 h-9 rounded-lg bg-emerald-500 text-white text-[10px] font-bold hover:bg-emerald-600 transition-colors disabled:opacity-50"
                  >
                    {actionLoading === mentor.id ? "Processing..." : "Approve Again"}
                  </button>
                )}
              </div>
            </div>
