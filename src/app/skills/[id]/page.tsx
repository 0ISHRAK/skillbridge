...
export default function CourseDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [selectedPayTab, setSelectedPayTab] = useState<"tokens" | "money">("tokens");
  ...
  useEffect(() => {
    async function loadData() {
      setIsLoading(true);
      try {
        const [courseRes, subRes, enrollRes, meRes] = await Promise.all([
        ...
        if (meRes?.ok) {
          ...
          if (meData.user?.tokenBalance !== undefined) {
            setUserTokenBalance(meData.user.tokenBalance);
          }
        }
      } catch {
        setNotFound(true);
  ...
  const handleTokenPay = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        setTransactionId(`TOKEN-${Math.floor(Math.random() * 899999 + 100000)}`);
        setPaymentStep("success");
        setIsEnrolled(true);
        if (data.tokenBalance !== undefined) setUserTokenBalance(data.tokenBalance);
      } else {
        setModalError(data.error || "Token payment failed.");
  ...
  const handlePinSubmit = async (e: React.FormEvent) => {
    ...
    setModalError("");
    try {
      const txnId = `SB-${paymentMethod?.toUpperCase() || "MONEY"}-${Math.floor(Math.random() * 899999 + 100000)}`;
      const res = await fetch("/api/courses/enroll", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
            ...
            {!isSubscribed && (
              <div>
                ...
                <p className="text-[10px] text-muted-foreground mt-1">
                  or pay with <span className="text-amber-500 font-bold">{Math.ceil(course.price / 10)} Skill Tokens</span>
                  <span className="ml-1 opacity-60">(1 Token = ৳10)</span>
                </p>
              </div>
            )}
            ...
            {isEnrolled ? (
              ...
              <div className="space-y-3">
                <button
                  ...
                  className="w-full h-11 flex items-center justify-center font-bold text-sm rounded-lg bg-primary text-primary-foreground hover:bg-primary/95 transition-all shadow-md shadow-primary/20 cursor-pointer"
                >
                  Enroll Now — Tokens or Money
                </button>
            ...
            {!isSubscribed && !isEnrolled && (
              <div className="text-center space-y-2">
                ...
                <div className="flex justify-center items-center gap-2 flex-wrap">
                  <span className="px-2 py-0.5 text-[9px] font-bold rounded bg-amber-500 text-zinc-950">🪙 Tokens</span>
                  <span className="px-2 py-0.5 text-[9px] font-bold rounded bg-rose-500 text-white">bKash</span>
                  <span className="px-2 py-0.5 text-[9px] font-bold rounded bg-orange-500 text-white">Nagad</span>
                  <span className="px-2 py-0.5 text-[9px] font-bold rounded bg-violet-600 text-white">Rocket</span>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="bg-zinc-950 border border-zinc-800 rounded-2xl max-w-sm w-full overflow-hidden shadow-2xl">
            <div className="px-5 py-4 border-b border-zinc-800 flex justify-between items-center bg-zinc-900/50">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-sm text-zinc-100">Course Checkout</h3>
              </div>
              <button onClick={() => setIsEnrollModalOpen(false)} className="text-zinc-400 hover:text-zinc-100 cursor-pointer">
            ...
            <div className="p-5 space-y-6">
              {paymentStep === "method" && (
                <div className="space-y-4">
                  <div className="text-center space-y-1">
                    <p className="text-xs text-zinc-400 font-medium">Select payment method for</p>
                    <p className="font-bold text-sm text-zinc-100 line-clamp-1">{course.title}</p>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 p-1 rounded-xl bg-zinc-900 border border-zinc-800">
                    <button
                      type="button"
                      onClick={() => setSelectedPayTab("tokens")}
                      className={`py-2 text-xs font-bold rounded-lg transition-all cursor-pointer ${
                        selectedPayTab === "tokens"
                    ...
                    <button
                      type="button"
                      onClick={() => setSelectedPayTab("money")}
                      className={`py-2 text-xs font-bold rounded-lg transition-all cursor-pointer ${
                        selectedPayTab === "money"
                  ...
                  {selectedPayTab === "tokens" ? (
                    <div className="space-y-3 p-4 rounded-xl bg-amber-500/5 border border-amber-500/20 text-center">
                      <div className="space-y-1">
                        <p className="text-lg font-extrabold text-amber-400">
                          {Math.ceil(course.price / 10)} Tokens
                        </p>
                        <p className="text-[10px] text-zinc-500">= ৳{course.price.toLocaleString()} BDT &nbsp;•&nbsp; 1 Token = ৳10</p>
                        <p className="text-[11px] text-zinc-400 mt-1">
                          Your balance: <span className={`font-bold ${userTokenBalance >= Math.ceil(course.price / 10) ? "text-emerald-400" : "text-red-400"}`}>{userTokenBalance} Tokens</span>
                          <span className="text-zinc-600 ml-1">(= ৳{(userTokenBalance * 10).toLocaleString()})</span>
                        </p>
                      </div>

                      {userTokenBalance >= Math.ceil(course.price / 10) ? (
                        <button
                          onClick={handleTokenPay}
                          disabled={confirming}
                          className="w-full h-10 bg-amber-500 text-zinc-950 font-bold text-xs rounded-lg hover:bg-amber-400 transition-all shadow-md cursor-pointer disabled:opacity-50"
                        >
                          {confirming ? "Processing Token Payment..." : `Pay ${Math.ceil(course.price / 10)} Tokens`}
                        </button>
                      ) : (
                        <div className="space-y-2">
                          <p className="text-[11px] text-red-400 font-semibold">
                            You need {Math.ceil(course.price / 10) - userTokenBalance} more Tokens.
                          </p>
                          <div className="flex gap-2">
                            <button
                              onClick={() => setSelectedPayTab("money")}
                              className="flex-1 py-2 bg-zinc-800 text-zinc-200 font-bold text-xs rounded-lg hover:bg-zinc-700 cursor-pointer"
                            >
                              Pay with Money
                            </button>
                            <Link
                              ...
                              className="flex-1 py-2 bg-amber-500 text-zinc-950 font-bold text-xs rounded-lg hover:bg-amber-400 text-center"
                            >
                              Buy Tokens →
                            </Link>
                          </div>
                    ...
                    <div className="grid grid-cols-1 gap-3">
                      <p className="text-xs text-zinc-400 text-center font-medium">
                        Pay <span className="text-primary font-bold">৳{course.price.toLocaleString()} BDT</span>
                      </p>
                      {(["bkash", "nagad", "rocket"] as const).map((m) => (
                        <button key={m} onClick={() => handleSelectMethod(m)}
                          className={`flex items-center justify-between p-3.5 rounded-xl border transition-all text-left text-zinc-200 group cursor-pointer ${
                        ...
                        >
                          <span className="font-bold text-xs flex items-center gap-2">
                            ...
                            {m.charAt(0).toUpperCase() + m.slice(1)} Mobile Banking
                          </span>
              ...
              {paymentStep === "number" && (
                <form onSubmit={handleNumberSubmit} className="space-y-4">
                  ...
                  />
                  <button type="submit" className="w-full h-10 flex items-center justify-center font-bold text-xs rounded-lg bg-zinc-100 text-zinc-950 hover:bg-zinc-200 transition-all cursor-pointer">
              ...
              {paymentStep === "pin" && (
                <form onSubmit={handlePinSubmit} className="space-y-4">
                  ...
                  <button type="submit" disabled={confirming}
                    className="w-full h-10 flex items-center justify-center font-bold text-xs rounded-lg bg-primary text-primary-foreground hover:bg-primary/95 transition-all shadow-md cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed"
                  >
              ...
              {paymentStep === "success" && (
                <div className="text-center space-y-4 py-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-zinc-400">Transaction ID: {transactionId}</p>
                  </div>
                ...
                <div className="text-center space-y-4 py-4">
                  ...
                  <div className="space-y-2 pt-2">
                    <button onClick={() => { setPaymentStep("method"); setModalError(""); }}
                      className="w-full h-9 flex items-center justify-center font-semibold text-xs rounded-md bg-zinc-100 text-zinc-950 hover:bg-zinc-200 transition-all cursor-pointer"
                    >
                      Try Again
                    </button>
