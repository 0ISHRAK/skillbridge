...
async function main() {
  ...
  const mentorProfiles = [
    {
      ...
      name: "Tanzim Hasan",
      bio: "Senior full-stack engineer and product builder helping learners ship production-ready web apps.",
      hourlyRate: 1500,
      skills: ["React", "Node.js", "Next.js", "System Design", "MongoDB"],
    },
    {
      email: "nabila@skillbridge.com",
      name: "Nabila Rahman",
      bio: "Product designer who turns complex workflows into clear, accessible experiences for growing teams.",
      hourlyRate: 1200,
      skills: ["Figma", "UX Research", "Design Systems", "Prototyping", "User Testing"],
    },
    {
      email: "farhan@skillbridge.com",
      name: "Farhan Ahmed",
      bio: "Freelance growth strategist and IELTS coach focused on practical career outcomes for Bangladesh-based learners.",
      hourlyRate: 1000,
      skills: ["IELTS", "SEO", "Upwork", "Content Strategy", "Client Communication"],
    },
  ];
  ...
  for (const profile of mentorProfiles) {
    ...
    mentors.push(mentor);
    console.log(`✅ Ready mentor: ${mentor.name} (${mentor.email})`);
  }
    ...
    if (existing) {
      await prisma.course.update({
        ...
        data: {
          ...
          price: course.price,
          level: course.title.includes("Advanced") ? "Advanced" : index % 3 === 0 ? "Intermediate" : "Beginner",
          duration: `${Math.max(4, Math.ceil(course.lessons.length / 2))} weeks`,
          thumbnail: thumbnails[index % thumbnails.length],
          whatYouLearn: JSON.stringify(course.lessons.slice(0, 5).map((lesson) => lesson.title)),
          requirements: JSON.stringify(["A laptop and stable internet", "Commit 3-5 hours per week"]),
